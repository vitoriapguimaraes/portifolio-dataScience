...
tabela = pd.read_csv('1. Cancelamento de Cartão\data-bank.csv', encoding="latin1")
...
caminho = (f'1. Cancelamento de Cartão/HISTOGRAM/{data_atual}')
...
for coluna in tabela:
    ...
    grafico.write_image(f"{caminho}/{coluna}.pdf")
    # grafico.show()
...
lista_histogramas = os.listdir(f"{caminho}/")
merger = PyPDF2.PdfMerger()
for histograma in lista_histogramas:
    merger.append(f"{caminho}/{histograma}") 
merger.write(f"{caminho}-Histogram.pdf")
